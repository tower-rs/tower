...
async fn retry_errors() {
    ...
    let (mut service, mut handle) = new_service(RetryErrors(None));
...
async fn retry_limit() {
    ...
    let (mut service, mut handle) = new_service(Limit(2, None));
...
async fn retry_error_inspection() {
    ...
    let (mut service, mut handle) = new_service(UnlessErr("reject", None));
...
#[derive(Clone)]
struct RetryErrors(Option<Req>);

impl Policy<Req, Res, Error> for RetryErrors {
    type Future = future::Ready<Option<Req>>;
    fn pre_request(&mut self, req: &mut Req) {
        self.0 = Some(*req);
    }

    fn retry(&mut self, result: &mut Result<Res, Error>) -> Self::Future {
        if result.is_err() {
            future::ready(Some(self.0.take().unwrap()))
        } else {
            future::ready(None)
        }
    }
...
impl Policy<Req, Res, Error> for Limit {
    ...
    fn retry(&mut self, result: &mut Result<Res, Error>) -> Self::Future {
        if result.is_err() && self.0 > 0 {
            self.0 -= 1;
            future::ready(Some(self.1.take().unwrap()))
        } else {
            future::ready(None)
        }
    }
...
#[derive(Clone)]
struct UnlessErr(InnerError, Option<Req>);
    ...
    fn retry(&mut self, result: &mut Result<Res, Error>) -> Self::Future {
        if let Err(err) = result {
            if err.to_string() != self.0 {
                return future::ready(Some(self.1.take().unwrap()));
            }
        }
...
impl Policy<Req, Res, Error> for CannotClone {
    type Future = future::Ready<Option<Req>>;
    fn retry(&mut self, _: &mut Result<Res, Error>) -> Self::Future {
...
//     fn retry(&mut self, result: &mut Result<Res, Error>) -> Option<Self::Future> {
//         if self.remaining == 0 {
