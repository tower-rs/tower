...
use tower::{util::ServiceExt, Call, Service};
...
async fn req_and_res() {
    ...
    let call = assert_ready_ok!(service.poll_ready());
    let mut response = task::spawn(call.call("hello"));
...
async fn clears_canceled_requests() {
    ...
    let call = assert_ready_ok!(service.poll_ready());
    let mut res1 = task::spawn(call.call("hello"));

    let send_response1 = assert_request_eq!(handle, "hello");

    // don't respond yet, new requests will get buffered
    let call = assert_ready_ok!(service.poll_ready());
    let res2 = task::spawn(call.call("hello2"));

    assert_pending!(handle.poll_request());

    let call = assert_ready_ok!(service.poll_ready());
    let mut res3 = task::spawn(call.call("hello3"));
...
async fn waits_for_channel_capacity() {
    ...
    handle.allow(0);
    let call = assert_ready_ok!(service.poll_ready());
    let mut response1 = task::spawn(call.call("hello"));
    assert_pending!(worker.poll());

    let call = assert_ready_ok!(service.poll_ready());
    let mut response2 = task::spawn(call.call("hello"));
    assert_pending!(worker.poll());

    let call = assert_ready_ok!(service.poll_ready());
    let mut response3 = task::spawn(call.call("hello"));
    ...
    let call = assert_ready_ok!(service.poll_ready());
    let mut response4 = task::spawn(call.call("hello"));
    assert_pending!(worker.poll());
...
async fn doesnt_leak_permits() {
    ...
    let call1 = assert_ready_ok!(task::spawn(service1.ready()).poll());
    // dropping these `call`s should not consume permits
    ...
    let mut response = task::spawn(call1.call(()));
    handle.allow(1);
    assert_pending!(worker.poll());
