...
#[test]
fn passthrough_sync() {
    let (mut service, mut handle) = new_service(10, |_| Ok::<_, Error>(()));
...
#[test]
fn rejected_sync() {
    let (mut service, _handle) = new_service(10, |_| Err::<(), Error>("err".into()));
...
fn saturate() {
    ...
    let (mut service, mut handle) = new_service(1, |_| Ok::<_, Error>(()));
