...
impl Ema {
    ...
    pub fn update(&mut self, timestamp: u64, value: f64) -> f64 {
        if self.timestamp == 0 {
            ...
            assert!(timestamp >= self.timestamp, "non monotonic timestamp detected");
...
mod tests {
    ...
    fn compute() {
        ...
        // it is what the `com.twitter.finagle.util.Ema` produces from `PER_DAY_DATA`
        const EMA_10_OVER_DAYS: [f64; 30] =
            [22.27, 22.26, 22.25, 22.24, 22.23, 22.22, 22.22, 22.24, 22.24, 22.25, 22.24, 22.25, 22.26,
                22.3, 22.4, 22.56, 22.67, 22.78, 22.89, 22.96, 23.04, 23.12, 23.17, 23.17, 23.17, 23.18,
                23.13, 23.13, 23.06, 22.98];

        let mut ema = Ema::new(10);

        let result = PER_DAY_DATA.iter()
                .enumerate()
